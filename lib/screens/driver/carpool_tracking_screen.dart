...
class CarpoolTrackingScreen extends StatefulWidget {
  ...
  const CarpoolTrackingScreen({Key? key, required this.carpoolRide})
    : super(key: key);
...
class _CarpoolTrackingScreenState extends State<CarpoolTrackingScreen> {
  ...
  void _initializeCarpoolRide() async {
    ...
    try {
      setState(() {
        _currentCarpoolRide = widget.carpoolRide;
      });
      await _loadRiders();
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading carpool ride: $e'),
  ...
  Future<void> _getRoutePolyline() async {
    try {
      ...
      for (int i = 0; i < _currentCarpoolRide!.stops.length - 1; i++) {
        final origin =
            '${_currentCarpoolRide!.stops[i].latitude},${_currentCarpoolRide!.stops[i].longitude}';
        final destination =
            '${_currentCarpoolRide!.stops[i + 1].latitude},${_currentCarpoolRide!.stops[i + 1].longitude}';
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Carpool status updated to ${_getStatusText(status)}',
            ),
              ...
              child: GoogleMap(
                ...
                initialCameraPosition: CameraPosition(
                  target: _currentCarpoolRide!.stops.isNotEmpty
                      ? LatLng(
                          _currentCarpoolRide!.stops[0].latitude,
                          _currentCarpoolRide!.stops[0].longitude,
                        )
                      : const LatLng(19.0760, 72.8777), // Mumbai, India
                  zoom: 15,
                ),
                markers: _markers,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed:
                            _currentStopIndex <
                                _currentCarpoolRide!.stops.length - 1
                            ? _nextStop
                            : null,
